...
def encrypt_text():
    ...
    for character in stringtoencrypt:
        ...
        if character in alpha:
            stringencrypted = stringencrypted + alpha[newposition]
			
        else:
            stringencrypted = stringencrypted + character
    enc_dec_text.insert(0, stringencrypted[::-1])
...
def decrypt_text():
    ...
    for character in stringtodecrypt:
        ...
        newposition = position-ciphershift
        if character in alpha:
            stringdecrypted = stringdecrypted + alpha[newposition]
        else:
            stringdecrypted = stringdecrypted + character  
    enc_dec_text.insert(0, stringdecrypted[::-1])
...
#, stringdecrypted = ' '.join(format(ord(x), 'b') for x in stringdecrypted